    ...
    <div>
        ...
        <el-dialog class="group-dialog" title="New Topic" :visible.sync="isTopicFormOpened"
            ...
            <el-form v-if="isTopicFormOpened" :model="topicForm" status-icon :rules="rules" ref="topicForm"
                ...
                <el-form-item>
                    <el-button v-if="updatingTopic" type="primary" @click="onTopicEvent" :disabled="creatingTopic">Update</el-button>
                    <el-button v-else type="primary" @click="onTopicEvent" :disabled="creatingTopic">Submit</el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
    </div>
...
<script>
    export default {
        name: 'TopicForm',
        props: ['creatingTopic', 'groupId', 'isUpdatingForm', 'updatingTopic', 'topic'],
        data() {
            return {
        ...
        methods: {
            ...
            onShowUpdateTopicForm() {
                this.topicForm = {...this.topicForm, title: this.topic.title, description: this.topic.text, priority: this.topic.priority, complexity: this.topic.complexity, id: this.topic.id};
                this.isTopicFormOpened = true;
            },
            onTopicEvent() {
