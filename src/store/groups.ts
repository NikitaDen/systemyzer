...
export default {
    ...
    createTopic(state: any, topic: any) {
      state.groups = [...state.groups].map(item => {
        if (item.id === topic.groupId) {
          return {
            ...item,
            topics: [...(item.topics ? item.topics : []), {...topic, id: topic.id}]
          }
        }
        return item
      })
          ...
          return {
            ...
            topics: [...item.topics].map(key => {
              if (key.id === topic.id) {
                return {
                  ...key,
                  links: {...key.links, [topic.linkId]: {link: topic.link, name: topic.name}}
                }
              }
              return key;
            })
  ...
  actions: {
    ...
    async createTopic({commit, dispatch}: any, {title, text, priority, complexity, groupId}: any) {
      try {
        const uid = await dispatch('getUid');
        const topic: any = await firebase.database().ref(`/users/${uid}/groups/${groupId}/topics`).push({
          title,
          text,
          done: false,
          groupId,
    ...
    async updateTopic({commit, dispatch}: any, {title, text, priority, complexity, groupId, id}: any) {
      try {
        const uid = await dispatch('getUid');
        await firebase.database().ref(`/users/${uid}/groups/${groupId}/topics`).child(id).update({
          title,
          text,
          priority,
          complexity
    ...
    async addTopicToFavorites({commit, dispatch}: any, {groupId, id, favorite}: any) {
      try {
        const uid = await dispatch('getUid');
        firebase.database().ref(`/users/${uid}/groups/${groupId}/topics`).child(id).update({favorite});
    ...
    async addLink({commit, dispatch}: any, {groupId, id, link, name}: any) {
      try {
        const uid = await dispatch('getUid');
        const newLink: any = await firebase.database().ref(`/users/${uid}/groups/${groupId}/topics/${id}/links`).push({
          link,
          name
        });
    ...
    async setTopicDone({commit, dispatch}: any, {groupId, id, done}: any) {
      try {
        const uid = await dispatch('getUid');
        firebase.database().ref(`/users/${uid}/groups/${groupId}/topics`).child(id).update({done});
